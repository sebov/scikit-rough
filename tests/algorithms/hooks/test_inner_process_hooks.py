...
def test_inner_process_hook_add_first_attr(
    ...
    state_fixture.values[HOOKS_DATA_X] = x
    state_fixture.values[HOOKS_DATA_X_COUNTS] = x_counts
    state_fixture.values[HOOKS_GROUP_INDEX] = GroupIndex.create_uniform(len(x))
    state_fixture.values[HOOKS_RESULT_ATTRS] = []
    ...
    for _ in range(len(attr_elements) + 2):
        ...
        assert state_fixture.values[HOOKS_RESULT_ATTRS] == expected_attrs
        actual_group_index: GroupIndex = state_fixture.values[HOOKS_GROUP_INDEX]
