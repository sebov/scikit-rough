...
family of subsets of $W$ such that $\bigcup \mathcal{S} = W$. The **Set Cover decision problem**
asks, for a given integer $l \geq 0$, whether there exists a subfamily
$\mathcal{C} \subseteq \mathcal{S}$ of size at most $l$ whose union also equals $W$:

$$\text{is there } \mathcal{C} \subseteq \mathcal{S} \text{ such that } |\mathcal{C}| \leq l
\text{ and } \bigcup \mathcal{C} = W \;?$$
