...
# NP-Hardness of Attribute-Simplest Correct Decision Bireduct Ensemble Problem

The problem of finding the attribute-simplest correct ensemble of decision bireducts (ASCDBEP) is
NP-hard. The proof is by polynomial reduction from the minimal dominating set problem.

## Statement

The Attribute-Simplest Correct Decision Bireduct Ensemble Problem (ASCDBEP) is NP-hard.

## Proof Strategy

The goal is a polynomial reduction from the **minimal dominating set problem** to ASCDBEP.
...
2. **(⇒) Feasibility.** If $\mathbb{G}$ has a dominating set of size $n$, then
   ...
   bireducts. The cost under $\prec_A$ is $n$ (the number of ones in the sorted cardinality
   sequence).
...
Since finding a minimum dominating set is NP-hard, ASCDBEP is NP-hard.
...
We show NP-hardness of ASCDBEP by polynomial reduction of the minimal dominating set problem.
...
### Single-Element Ensemble is Correct but Not Attribute-Simplest
...
However, we can always construct an attribute-simpler (or equally attribute-simple if
$B_{\mathbb{G}}$ is already a singleton) correct ensemble.

### Constructing an Attribute-Simpler Ensemble
...
### Attribute-Simplicity Corresponds to Dominating Set Size

The constructed ensemble has attribute cardinalities $(1, 1, \ldots, 1, 0, 0, \ldots, 0)$ (with $n$
ones and $n-1$ zeros). According to the attribute-simplicity order $\prec_A$, this is
attribute-simpler than any ensemble with a larger maximum cardinality.

**Why is this the attribute-simplest possible?** We must show two things: (a) no correct ensemble
can have fewer than $n$ bireducts with non-empty attribute sets, and (b) no correct ensemble with
exactly $n$ bireducts with non-empty attribute sets can have fewer than $n-1$ bireducts with empty
attribute sets.
...
(all vertices in $B$). The constructed ensemble with $n$ ones is therefore the attribute-simplest
possible.
...
$k > n - 2$, i.e., $k \ge n - 1$. More zeros would make the ensemble more complex under $\prec_A$
...
Therefore, the attribute-simplest correct ensemble corresponds to the smallest dominating set.

### Conclusion

The attribute-simplest correct ensemble of decision bireducts corresponds to the smallest dominating
set in the graph $\mathbb{G}$. Since finding the minimal dominating set is NP-hard, ASCDBEP is
NP-hard.

## Remarks

This proof shows that ensemble optimization is computationally intractable, even for the
attribute-based notion of simplicity (sorted attribute cardinalities).
