...
# # from skrough_old.base import RoughBase
...
# # from skrough.struct import Reduct


# class GreedyHeuristicReduct(RoughBase, DecTableOpsMixin):

#     def __init__(self, epsilon=0.0, n_candidate_attrs=None, score_func=gini_impurity,
# random_state=None):
#         super(GreedyHeuristicReduct, self).__init__(score_func,
...
#         scores = np.fromiter((self.split_groups_and_compute_chaos_score(
# group_index, n_groups, i, x, x_count_distinct, y, y_count_distinct)
...
# self.x, self.y, self.y_count_distinct)
...
# self.n_candidate_attrs]),
#                                                    replace=False)
#             best_attr = self.get_best_attr(group_index, n_groups, candidate_attrs,
# self.x, self.x_count_distinct, self.y, self.y_count_distinct)
